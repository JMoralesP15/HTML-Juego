...
async function boot(page,{width=390,height=844,start=true}={}){
  ...
  await page.goto(url);
  await page.waitForFunction(()=>typeof __QA_V14__==='object'&&typeof __QA_V14_CURATION__==='object');
...
test('las 300 fechas reciben score y nivel de cultura general',async({page})=>{
  ...
  const result=await page.evaluate(()=>({total:QUESTIONS.length,invalid:QUESTIONS.filter(q=>!['core','context','niche'].includes(q.cultureTier)||!Number.isFinite(q.cultureScore)||q.cultureScore<0||q.cultureScore>100).length,context:QUESTIONS.filter(q=>!q.v14Context?.remember||!q.v14Context?.connection).length,summary:__QA_V14_CURATION__.summary}));
  expect(result.total).toBe(300);expect(result.invalid).toBe(0);expect(result.context).toBe(0);
  expect(Object.values(result.summary.tiers).reduce((a,b)=>a+b,0)).toBe(300);
});

test('ambiente generativo está apagado por defecto y sólo parte por gesto',async({page})=>{
  await boot(page,{start:false});
  const button=page.locator('#ambientToggle');await expect(button).toHaveAttribute('aria-pressed','false');await expect(button).toContainText('Ambiente: no');
  await button.click();await expect(button).toHaveAttribute('aria-pressed','true');await expect(button).toContainText('Ambiente: sí');
  await button.click();await expect(button).toHaveAttribute('aria-pressed','false');
});

test('feedback móvil no desborda y muestra la clasificación editorial',async({page})=>{
  await boot(page,{width:375,height:667});await answer(page);
  await expect(page.locator('.v14-culture-badge')).toBeVisible();
  ...
  const toggle=page.locator('[data-v13-action="context-toggle"]');await toggle.click();await expect(page.locator('.v13-context-extra')).toContainText('DATO PARA RECORDAR');await expect(page.locator('.v13-context-extra')).toContainText('CONEXIÓN');
});

test('Commons sólo acepta licencias abiertas con atribución visible',async({page})=>{
  await boot(page,{width:390,height:844});
  const q=await page.evaluate(()=>{const x=QUESTION_BY_ID.get(round.questionIds[round.index]);return {id:x.id,title:x.title}});
  ...
  await answer(page);await expect(page.locator('.v14-open-media-credit')).toBeVisible({timeout:7000});await expect(page.locator('.v14-open-media-credit')).toContainText('CC BY 4.0');await expect(page.locator('.v14-open-media-credit a')).toHaveAttribute('href',/commons\.wikimedia\.org/);
...
test('fallo de red conserva el fallback documental offline',async({page})=>{
  ...
  await page.evaluate(()=>{const q=QUESTIONS.find(x=>x.v12GeneratedImage);round=createRound('practice',[q]);currentView='repaso';renderGame();setYear(q.year+2);commitAnswer(false)});
  const before=await page.locator('.atlas-document-image img').getAttribute('src');expect(before).toBeTruthy();
  await page.waitForTimeout(900);const after=await page.locator('.atlas-document-image img').getAttribute('src');expect(after).toBe(before);
});