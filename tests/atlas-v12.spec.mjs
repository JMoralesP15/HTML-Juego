...
async function boot(page,size=desktop){
  ...
  await page.setViewportSize(size);await page.goto(url);await page.waitForFunction(()=>typeof startDaily==='function'&&typeof __QA_TIMER__==='object'&&typeof __QA_V15__==='object');
...
test('feedback conserva documento y contexto bajo demanda',async({page})=>{await boot(page);await answerOffset(page,0);await expect(page.locator('.atlas-document')).toBeHidden();await page.locator('[data-v15-action="context-toggle"]').click();await expect(page.locator('.atlas-document')).toBeVisible();await expect(page.locator('.atlas-document-copy')).toContainText('QUÉ OCURRIÓ');await expect(page.locator('.atlas-document-copy')).toContainText('EN EL MAPA DEL TIEMPO');await page.screenshot({path:'test-results/screenshots/v15-contexto-ampliado.png',fullPage:true})});
