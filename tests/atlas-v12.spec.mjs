...
async function boot(page,size=desktop){
  ...
  await page.setViewportSize(size);await page.goto(url);await page.waitForFunction(()=>typeof startDaily==='function'&&typeof __QA_TIMER__==='object');
...
test('visibility hidden pausa el temporizador y visible lo reanuda',async({page})=>{await boot(page);await page.evaluate(()=>{Object.defineProperty(document,'visibilityState',{configurable:true,get:()=>window.__testVisibility||'visible'});window.__testVisibility='hidden';document.dispatchEvent(new Event('visibilitychange'))});const a=await page.evaluate(()=>__QA_TIMER__.snapshot().remainingMs);await page.waitForTimeout(350);const b=await page.evaluate(()=>__QA_TIMER__.snapshot().remainingMs);expect(Math.abs(a-b)).toBeLessThan(40);await page.evaluate(()=>{window.__testVisibility='visible';document.dispatchEvent(new Event('visibilitychange'))});await page.waitForTimeout(250);const c=await page.evaluate(()=>__QA_TIMER__.snapshot().remainingMs);expect(c).toBeLessThan(b)});

test('timeout autoenvía la estimación de forma neutral',async({page})=>{await boot(page);await page.evaluate(()=>__QA_TIMER__.setRemaining(30));await page.waitForTimeout(220);const a=await page.evaluate(()=>round.answers[0]);expect(a.timedOut).toBe(true);expect(a.remainingMs).toBe(0);expect(a.timeBonus).toBe(0);await expect(page.locator('body')).toContainText('Tiempo agotado · registramos tu estimación.')});

test('No lo sé cancela timer, puntaje y bonus',async({page})=>{await boot(page);await page.evaluate(()=>commitAnswer(true));const a=await page.evaluate(()=>round.answers[0]);expect(a.skipped).toBe(true);expect(a.points).toBe(0);expect(a.timeBonus).toBe(0);await expect(page.locator('.atlas-state-revealed')).toBeVisible();await expect(page.locator('.reveal-scale')).toBeVisible()});
...
test('estado EXACTA tiene identidad propia',async({page})=>{await boot(page);await answerOffset(page,0);await expect(page.locator('.atlas-state-exact')).toBeVisible();await expect(page.locator('.atlas-sigil.exact')).toBeVisible();await expect(page.locator('.exact-scale')).toBeVisible();await page.screenshot({path:'test-results/screenshots/v12-feedback-exacta-1440x900.png',fullPage:true})});

test('estado CERCA tiene identidad propia',async({page})=>{await boot(page);await answerOffset(page,1);await expect(page.locator('.atlas-state-near')).toBeVisible();await expect(page.locator('.scale-near')).toBeVisible();await page.screenshot({path:'test-results/screenshots/v12-feedback-cerca-1440x900.png',fullPage:true})});

test('estado DESFASE tiene identidad propia',async({page})=>{await boot(page);await answerOffset(page,5);await expect(page.locator('.atlas-state-medium')).toBeVisible();await expect(page.locator('.scale-close')).toBeVisible();await page.screenshot({path:'test-results/screenshots/v12-feedback-desfase-1440x900.png',fullPage:true})});

test('estado SALTO TEMPORAL tiene identidad propia',async({page})=>{await boot(page);await answerOffset(page,40);await expect(page.locator('.atlas-state-far')).toBeVisible();await expect(page.locator('.scale-wide')).toBeVisible();await page.screenshot({path:'test-results/screenshots/v12-feedback-salto-1440x900.png',fullPage:true})});
...
test('feedback integra imagen y contexto en una sola composición',async({page})=>{await boot(page);await answerOffset(page,0);await expect(page.locator('.atlas-document')).toBeVisible();await expect(page.locator('.atlas-document-copy')).toContainText('QUÉ OCURRIÓ');await expect(page.locator('.atlas-document-copy')).toContainText('EN EL MAPA DEL TIEMPO');await page.screenshot({path:'test-results/screenshots/v12-contexto-ampliado.png',fullPage:true})});
