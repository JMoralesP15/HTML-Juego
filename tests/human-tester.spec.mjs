...
test('default release gates all pools and keeps a five-event session across reload',async({page})=>{
  ...
  await page.reload();await page.getByRole('button',{name:'Continuar mi partida →'}).click();expect(await page.evaluate(()=>round.questionIds)).toEqual(ids);
  for(let i=0;i<5;i++){await page.evaluate(()=>{qaHumanBegin();commitAnswer(true)});await expect(page.locator('.friendly-idea')).toBeVisible();await page.evaluate(()=>nextQuestion())}
...
for(const width of [375,390])test(`approved copy and chosen image visible before expanding at ${width}px`,async({page})=>{
  await boot(page,width);await page.evaluate(()=>{startPractice('all','Todas',5);round.questionIds[0]='bitcoin';renderGame();qaHumanBegin();commitAnswer(true)});
  const approved=await page.evaluate(()=>({...QUESTION_BY_ID.get('bitcoin').approvedLearning,brief:qaHumanLearning(QUESTION_BY_ID.get('bitcoin')).brief}));
  await expect(page.locator('.friendly-idea').first()).toHaveText(approved.brief);
