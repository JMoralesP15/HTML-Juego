import vm from 'node:vm';
import fs from 'node:fs';
import assert from 'node:assert/strict';
const context={window:{},document:{querySelector:()=>null,addEventListener:()=>{}},openDetail:()=>{}};
vm.createContext(context);vm.runInContext(fs.readFileSync(new URL('../js/learning-v16.js',import.meta.url),'utf8'),context);
const api=context.window.__QA_V16__;
const q={fact:'El régimen autoritario portugués cayó en abril de 1974.',context:'La fecha concreta registrada para el hito es el 25 de abril de 1974. La misma ficha sitúa el hito en Portugal.',significance:'La misma ficha sitúa el hito en Portugal.'};
const narrative=api.learningNarrative(q).join(' ');
assert.equal(narrative,q.fact);
assert.equal(api.learningFor(q).context,'');
assert.equal(api.learningNarrative({fact:'Un avance real.',context:'Un avance real.',significance:'Un avance real.'}).length,1);
assert.equal(api.learningNarrative({fact:'WhatsApp se lanzó en 2009.',context:'El lanzamiento figura fechado el enero de 2009. La ficha identifica al desarrollador como WhatsApp LLC.'}).join(' '),'WhatsApp se lanzó en 2009.');
console.log('4 learning cleanup assertions passed');
const evidenceContext={window:{}};vm.createContext(evidenceContext);
vm.runInContext(fs.readFileSync(new URL('../js/content.js',import.meta.url),'utf8')+';globalThis.before=QUESTIONS.map(q=>({id:q.id,source:q.source,context:q.context}));',evidenceContext);
vm.runInContext(fs.readFileSync(new URL('../js/editorial-verification-v18.js',import.meta.url),'utf8')+';globalThis.after=QUESTIONS;',evidenceContext);
assert.equal(evidenceContext.after.find(q=>q.id==='jurassic').source,evidenceContext.before.find(q=>q.id==='jurassic').source);
assert.equal(evidenceContext.after.filter(q=>q.context!==evidenceContext.before.find(x=>x.id===q.id).context).length,0);
console.log('2 evidence isolation assertions passed');
