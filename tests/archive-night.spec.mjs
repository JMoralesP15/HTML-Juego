...
test('Feedback revelado es neutral y no punitivo',async({page})=>{await boot(page,desktop);await page.evaluate(()=>commitAnswer(true));await expect(page.locator('.v15-result-distance')).toHaveText('Fecha revelada para repasar');await expect(page.locator('.v15-result-note')).toContainText('Queda guardada para repaso');await expect(page.locator('body')).not.toContainText('fallaste');await page.screenshot({path:'test-results/screenshots/v15-feedback-revelada-1440x900.png',fullPage:true})});
