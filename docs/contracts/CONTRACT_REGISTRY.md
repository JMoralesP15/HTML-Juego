# QUÉ AÑO — Contract Registry v1.8.5
...
| 11 | SCORING_TIMER | `atlas-v12.js` | storage normaliza campos temporales; v1.5 sólo consume snapshot para telemetría | UNIQUE | DIRECT | `atlas-v12.spec`, `v15-simplification.spec`, QA run 34735182507 | mantener fórmula `timer-v1`; cualquier cambio exige nueva versión contractual |
...
| 17 | QA_RELEASE | workflow + suites | snapshots, reports y Playwright | SHARED | INDIRECT | `.github/workflows/qa.yml` | mantener captura determinista y artifacts; no regenerar baselines automáticamente |
...
- Propietario único sin competidor cargado conocido: 7/18 (`CONTENT_BANK`, `SCHEDULER`, `CALENDAR`, `STORAGE`, `SCORING_TIMER`, `NAVIGATION`, `ANALYTICS`).
- `ownership_ratio = 7 / 18 = 0.3889`.
- Protección DIRECT observable: 14/18.
- `test_protection_ratio = 14 / 18 = 0.7778`.
- Runtime v1.8.5 observado en CI: 27 colisiones estáticas, 13 behavior patches, 0 MutationObservers, 7 render subscriptions.
...
`OBSOLETE_CANDIDATE` no significa eliminable. La retirada exige: owner sustituto explícito, test de paridad del comportamiento, QA visual determinista y verificación de persistencia cuando corresponda.
