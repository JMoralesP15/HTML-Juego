...
const baseline=readJson('config/architecture-budget-v185.json');
...
const report={schema:'que-ano-architecture-budget-result-v1.8.5',baseline:baseline.baseline,policy:baseline.policy,current,checks,pass:failed.length===0,failed};
fs.mkdirSync(path.join(root,'reports'),{recursive:true});
fs.writeFileSync(path.join(root,'reports','architecture-budget-v185.json'),JSON.stringify(report,null,2)+'\n');
console.log(JSON.stringify(report,null,2));
if(failed.length){
  console.error(`Architecture budget failed: ${failed.map(x=>`${x.metric}=${x.value} (${x.kind} ${x.limit})`).join(', ')}`);
  process.exitCode=1;
}
