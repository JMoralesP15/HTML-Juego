...
const baseline=readJson('config/architecture-budget-v187.json');
...
const checks=[];
for(const [metric,limit] of Object.entries(baseline.max||{})){const value=current[metric];checks.push({metric,kind:'max',limit,value,pass:Number.isFinite(value)&&value<=limit})}
for(const [metric,limit] of Object.entries(baseline.min||{})){const value=current[metric];checks.push({metric,kind:'min',limit,value,pass:Number.isFinite(value)&&value+Number.EPSILON>=limit})}
const failed=checks.filter(x=>!x.pass);
const report={schema:'que-ano-architecture-budget-result-v1.8.7',baseline:baseline.baseline,policy:baseline.policy,current,checks,pass:failed.length===0,failed};
fs.mkdirSync(path.join(root,'reports'),{recursive:true});
fs.writeFileSync(path.join(root,'reports','architecture-budget-v187.json'),JSON.stringify(report,null,2)+'\n');
