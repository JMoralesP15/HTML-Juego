...
import {matchIdentity,identityTokens,photoMime,captureYear} from './media-identity.mjs';
...
const requested=process.argv.find(x=>x.startsWith('--ids='))?.slice(6).split(',').filter(Boolean);
const ids=requested||questions.map(q=>q.id);
...
const referenceVisual=/\b(logo|logotype|poster|afiche|cover|portada|caratula|screenshot|screen ?capture|main page|homepage|web page|user interface|album art|box art|game cover|book cover)\b/i;
...
async function api(base,params,attempt=1){
  ...
  try{
    const res=await fetch(url,{signal:AbortSignal.timeout(20000),headers:{'User-Agent':UA,'Api-User-Agent':UA,'Accept':'application/json'}});
    if(!res.ok){const e=new Error(`HTTP ${res.status}`);e.status=res.status;throw e}
    const data=await res.json();if(data.error)throw new Error(data.error.info||data.error.code);return data;
  }catch(error){
...
function visualType(q,blob){return referenceVisual.test(blob)?'review_only_reference':'contextual_photo'}
async function commonsSearch(q){
  ...
  for(const page of data?.query?.pages||[]){
    const info=page.imageinfo?.[0],meta=info?.extmetadata||{};if(!info?.url||!photoMime(info.mime))continue;
    ...
    const oy=originalYear(meta),s=semanticScore(q,blob,oy);if(!s.identityMatch||s.score<4)continue;
    // A single name in description may only be an incidental Google Maps credit.
    if(identityTokens(q).length===1&&!matchIdentity(q,page.title).accepted)continue;
    ...
    const warnings=['capture_date_requires_review'];if(!Number.isFinite(oy))warnings.push('image_year_unknown');if(Number.isFinite(s.delta)&&Math.abs(s.delta)>12)warnings.push('high_temporal_delta');if(referenceVisual.test(blob))warnings.push('reference_asset');if(rightsTier!=='publishable')warnings.push('rights_unresolved');if(s.matched.length===1)warnings.push('single_alias_match');
    out.push({provider:'wikimedia_commons',fileTitle:page.title,src:info.thumburl||info.url,original:info.url,sourcePage:`https://commons.wikimedia.org/wiki/${encodeURIComponent(page.title.replace(/ /g,'_')).replace(/%3A/g,':')}`,artist:ext(meta,'Artist')||null,credit:ext(meta,'Credit')||null,license,licenseUrl:ext(meta,'LicenseUrl')||null,description:description||null,visualType:type,rightsTier,eventYear:q.year,imageYear:oy,dateEvidence:'file_metadata_unverified',temporalDeltaYears:s.delta,matchedAliases:s.matched,score:s.score,rejectionWarnings:warnings,reviewRequired:true});
  }
...
const previousPath=path.join(root,'reports/editorial-review-media-v187c.json');
const previous=requested&&fs.existsSync(previousPath)?JSON.parse(fs.readFileSync(previousPath,'utf8')):{items:{}};
const items={...previous.items};let searched=0,found=0,noCandidate=0,errors=0,totalCandidates=0,publishable=0,reviewOnly=0;
async function discover(id){const q=byId.get(id);if(!q){items[id]={status:'missing_question',candidates:[],queryAttempts:[]};errors++;return}searched++;let attempts=[],candidates=[];try{const c=await commonsSearch(q);attempts.push(...c.attempts);candidates.push(...c.candidates);if(candidates.length<4){const es=await wikipediaSearch(q,'es');attempts.push(...es.attempts);candidates.push(...es.candidates)}if(candidates.length<4){const en=await wikipediaSearch(q,'en');attempts.push(...en.attempts);candidates.push(...en.candidates)}}catch(e){attempts.push({source:'unexpected',query:q.title,status:'error',error:String(e.message)})}
...
// Three read-only workers keep network demand bounded.
const queue=[...ids];
await Promise.all(Array.from({length:3},async()=>{while(queue.length){const id=queue.shift();await discover(id)}}));
