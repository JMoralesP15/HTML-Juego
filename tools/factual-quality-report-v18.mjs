// QUÉ AÑO v1.8 — static quality gate for generated factual evidence, explicit manual review and media.
...
for(const name of ['content','editorial','editorial-verification-v18','editorial-v18-manual'])vm.runInContext(fs.readFileSync(path.join(root,'js',`${name}.js`),'utf8'),ctx,{filename:`${name}.js`});
...
const rows=questions.map(q=>{const e=evidence[q.id];const manualOverlay=Boolean(q.editorialVerified&&String(q.editorialVerificationVersion||'').startsWith('v1.8-manual-'));const status=q.editorialVerified?'manual_verified':e?.status||'missing';return {q,e,status,manualOverlay,sourceLabel:q.editorialVerified?q.sourceLabel:e?.sourceLabel,sourceUrl:q.editorialVerified?q.source:e?.sourceUrl}});
...
const autoPromotions=rows.filter(x=>x.q.editorialVerified&&!x.e?.manualVerified&&!x.manualOverlay).map(x=>x.q.id);
const manualLost=rows.filter(x=>x.e?.manualVerified&&x.status!=='manual_verified').map(x=>x.q.id);
const genericLabels=rows.filter(x=>/referencia\s+(general|heredada)|fuente\s+general/i.test(clean(x.sourceLabel))).map(x=>x.q.id);
