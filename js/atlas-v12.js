...
function qaHumanRender(q){
  ...
  let body,footer;
  if(answering){
    body=`<header class="friendly-progress"><span>Pregunta ${round.index+1} de ${round.questionIds.length}</span>${ready?qaTimerHTML():'<span>Sin prisa para leer</span>'}</header><div class="friendly-question"><p class="friendly-topic" aria-hidden="true">${esc(q.title)}</p><h1 id="questionTitle">${esc(q.prompt)}</h1></div>`;
